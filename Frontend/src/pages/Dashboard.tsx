...
const quickActions = [
  { label: 'Adaugă cheltuială', sub: 'Înregistrează manual o sumă',   nav: '/add-expense?mode=manual', dark: true,
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const totalLuna = expenseLuna.reduce((sum, e) => sum + Number(e.amount), 0)
        ...
        const totalLunaT = expenseLunaT.reduce((sum, e) => sum + Number(e.amount), 0)
        ...
        last7.forEach((d) => { d.h = Math.max(Math.round((d.amount / maxAmt) * 100), 4) })
        setBarData(last7)
